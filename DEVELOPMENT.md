...
## M5 第一批（2026-09-01 晚）：播放器功能完整化

- **自动连播**（修复隐藏 bug：此前单文件播完即停）：completed 事件 →
  按循环模式切下一首
- **循环模式**三态循环切换（off / all / one，AppBar 按钮状态图标）
- **播放队列弹窗**（PRD §5.6.5）：r18 顶圆角（全局 BottomSheet 主题 20），
  0.7 屏高，ReorderableListView 拖拽排序（onReorderItem），左滑删除，
  当前曲高亮，点击跳播
- **倍速**：AppBar 速度按钮 + 滑杆弹窗（0.5–2.5 步进 0.25，恢复 1.0×）
- **睡眠定时**（PRD §5.6.6）：预设 5/10/15/30/60/120 分钟 + 24 小时制
  TimePicker 指定时刻 + 取消；到点 3 秒淡出后暂停（验收要求）
- **装机流程修正**：覆盖安装后 force-stop 杀旧进程（旧进程仍跑旧代码的坑）
...
## M5 三期（2026-09-01 深夜收尾）

- **网络相关推荐**（用户需求）：本地详情页推荐位 = 本地同社团（前）+
  asmr.one 同社团在线作品（后，circleId 路径 getCircleWorks），4:3 网络封面卡，
  点击进在线详情；失败静默回退纯本地推荐
- **网络参考区移至文件树上方**（用户指定顺序：本地标题/信息 → 网络参考 → 文件树 → 推荐）
- 实机验证：RJ416816 详情页网络推荐 4 张卡渲染 ✓、参考区位置 ✓

## M5 完成度（PRD §10 里程碑验收对照）

✅ 已完成：自动连播、循环三态、播放队列（拖拽/左滑/跳播）、倍速 0.5-2.5、
睡眠定时（预设+指定时刻+淡出）、歌词 seek 联动（本地外推+精确滚动）、
歌词行点击跳转、悬浮桌面歌词（穿透+可拖+全局化）、字体大小调整（全局+歌词）
...
## PRD 对照收尾（2026-09-02 凌晨，ba2809a）

- **M6 搜索完整版**：五条件类型（关键词/RJ/标签/社团/声优——后三类双源：
  本地字段 + NetMeta 缓存）+ 搜索历史（10 条/回填/清空）
- **音频增益**（PRD §5.6.3/§5.10）：设置页 −12~+12dB 步进 0.5 滑杆，
  dB→倍率映射 setVolume 实时生效（+12dB≈4× 实验性）

### PRD 验收清单最终状态

#1-#21 全部主体实现；细节待真机长测验证。剩余明确欠账：
字幕库页、FTS、链接导入、暗色专项、视觉比对专项（需用户配合）。

## 欠账清偿完成（2026-09-02 00:15，651d541）

- **播放列表链接导入**（PRD §5.8 双段弹窗）：粘贴分享文本 → RJ 正则提取 →
  匹配本地作品批量加歌
- **字幕库页**（M7 第四 Tab）：全库已关联 lrc/vtt/srt 音轨清单，点击定位播放

### 欠账清单最终状态

✅ 已清：播放列表管理、链接导入、字幕库、标签过滤、DLsite 兜底、
封面中英日关键词、搜索五条件+筛选+排除语法、音频增益、字体调整、
悬浮歌词、权限引导
⏳ 剩余（均为优化项）：FTS（LIKE 已够用）、暗色专项复查、
视觉比对专项（需用户配合）、增益正值削波优化

## 用户指定清单实施（2026-09-02 00:30，53e947b）

- 权限统一检查（存储+通知，启动弹窗一次性引导）
- 在线书签 UI（详情页按钮 + 在线墙收藏筛选，乐观更新/游客降级）
- 卡片字号联动（≥1.5× 网格模式只留封面）
- Google gtx 翻译（标题译文，不可用静默）
- DLsite 外链按钮；偏好分组（元数据开关/仅Wi-Fi/字幕默认/缓存清理）
- 增量扫描（目录签名跳过元数据解析）

清单剩余未做：#2 在线分类Tab+高级筛选（中等工作量，下一批）

## 指定清单 #2 收官（2026-09-02 00:35，c4f04ef）

- 在线分类 chips（全部/全年龄/R18/带字幕，服务端过滤）
- 偏好行为接线（元数据开关拦截拉取 / 仅 Wi-Fi 网络类型检查 /
  字幕默认显示初始视图）
- **用户指定清单全部完成**：权限统一检查、#1 书签、#2 分类筛选、
  #3 偏好分组、翻译（gtx）、#7 外链、#8 增量扫描、#11 卡片联动
